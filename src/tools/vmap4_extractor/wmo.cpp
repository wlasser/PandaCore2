...
int WMOGroup::ConvertToVMAPGroupWmo(FILE *output, WMORoot *rootWMO, bool pPreciseVectorData)
{
    ...
    else
    {
        ...
        int nColVertices = 0;
        for (uint32_t_t i=0; i<nVertices; ++i)
        {
            if (IndexRenum[i] == 1)
            {
    ...
    if(LiquEx_size != 0)
    {
        ...
        for (uint32_t_t i = 0; i<LiquEx_size/sizeof(WMOLiquidVert); ++i)
...
WMOInstance::WMOInstance(MPQFile &f,const char* WmoInstName, uint32_t_t mapID, uint32_t_t tileX, uint32_t_t tileY, FILE *pDirfile)
{
    pos = Vec3D(0,0,0);
    ...
    float scale = 1.0f;
    uint32_t_t flags = MOD_HAS_BOUND;
    if(tileX == 65 && tileY == 65) flags |= MOD_WORLDSPAWN;
    //write mapID, tileX, tileY, Flags, ID, Pos, Rot, Scale, Bound_lo, Bound_hi, name
    fwrite(&mapID, sizeof(uint32_t_t), 1, pDirfile);
    fwrite(&tileX, sizeof(uint32_t_t), 1, pDirfile);
    fwrite(&tileY, sizeof(uint32_t_t), 1, pDirfile);
    fwrite(&flags, sizeof(uint32_t_t), 1, pDirfile);
    fwrite(&adtId, sizeof(uint16_t), 1, pDirfile);
    fwrite(&id, sizeof(uint32_t_t), 1, pDirfile);
    fwrite(&pos, sizeof(float), 3, pDirfile);
    fwrite(&rot, sizeof(float), 3, pDirfile);
    fwrite(&scale, sizeof(float), 1, pDirfile);
    fwrite(&pos2, sizeof(float), 3, pDirfile);
    fwrite(&pos3, sizeof(float), 3, pDirfile);
    uint32_t_t nlen=strlen(WmoInstName);
    fwrite(&nlen, sizeof(uint32_t_t), 1, pDirfile);
